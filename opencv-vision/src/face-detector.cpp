...
FaceDetector::FaceDetector(const std::string& face_cascade_path,
  ...
  if (!face_cascade.load(face_cascade_path)) {
    std::cerr << "Error loading face cascade: " << face_cascade_path
              << std::endl;
  }

  if (!eyes_cascade.load(eyes_cascade_path)) {
    std::cerr << "Error loading eyes cascade: " << eyes_cascade_path
              << std::endl;
  }
}
...
void FaceDetector::drawDetections(cv::Mat& image,
...
bool FaceDetector::isInitialized() const { return !face_cascade.empty(); }
