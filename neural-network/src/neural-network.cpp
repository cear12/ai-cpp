...
double NeuralNetwork::SigmoidDerivative(double x) { return x * (1.0 - x); }

double NeuralNetwork::Relu(double x) { return std::max(0.0, x); }

double NeuralNetwork::ReluDerivative(double x) { return x > 0 ? 1.0 : 0.0; }
...
std::vector<double> NeuralNetwork::FeedForward(
  ...
  for (size_t i = 1; i < topology_.size(); ++i) {
    for (int j = 0; j < topology_[i]; ++j) {
      ...
      // Apply activation function (sigmoid for hidden layers, linear for
      // output)
...
std::vector<double> NeuralNetwork::BackPropagate(
  ...
  for (int epoch = 0; epoch < epochs; ++epoch) {
    ...
    if (epoch % 100 == 0) {
      std::cout << "Epoch " << epoch
                << ", Error: " << total_error / inputs.size() << std::endl;
    }
  }
}
...
void NeuralNetwork::LoadModel(const std::string& filename) {
  ...
  if (loaded_topology != topology_) {
    std::cerr << "Error: " << filename
              << " topology does not match this network's topology"
              << std::endl;
    return;
  }
  ...
  if (!in) {
    std::cerr << "Error: " << filename << " was truncated or malformed"
              << std::endl;
  }
}
